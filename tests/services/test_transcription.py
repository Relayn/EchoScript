    ...
    model.transcribe.side_effect = [
        {
            ...
            "segments": [{"start": 0, "end": 5, "text": " Это первая часть. "}],
        },
        {
            "text": "Это вторая часть.",
            "segments": [{"start": 30, "end": 35, "text": " Это вторая часть."}],
        },
    ]
    return model
...
def test_transcribe_returns_dict_with_text_and_segments(
    ...
    assert result["text"] == "Это первая часть. Это вторая часть."
...
def test_transcribe_with_progress_callback(mock_whisper_model, mock_sound_file):
    ...
    mock_callback.assert_has_calls(
        [call(1, 2), call(2, 2)]  # (обработано_чанков, всего_чанков)
    )
